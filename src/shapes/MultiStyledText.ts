...
export class MultiStyledText extends Shape<MultiStyledTextConfig> {
  ...
  private computeTextParts() {
    ...
    const additionalWidth = shouldAddEllipsis
      ? this.measurePart({ text: ellipsis, style: styles[styles.length - 1] })
      : 0;
    ...
    const findParts = (start: number, end: number) => {
      ...
      for (const char of chars) {
        const similarGroupIndex = parts.findIndex(
          (part) => part.style === char.style
        );
    ...
    for (const line of this.textLines) {
      ...
      for (const part of line.parts) {
        ...
        if (this.letterSpacing() !== 0 || this.align() === 'justify') {
          ...
          part.x = lineX;
          part.y = y + lineY;
          part.height = part.style.fontSize * this.lineHeight();
          context.fillStrokeShape(this);
